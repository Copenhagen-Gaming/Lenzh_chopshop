...
ESX.RegisterServerCallback('Lenzh_chopshop:OwnedCar', function(source, cb, plate, owner, garage_id, impound)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    ...
    MySQL.Async.fetchAll("SELECT * FROM owned_vehicles WHERE plate=@plate AND owner=@identifier",{['@identifier'] = xPlayer.getIdentifier(), ['@plate'] = plate}, function(result)
        if result[1] and Config.AnyoneCanChop == true then
            ...
            Citizen.Wait(5)
            MySQL.Async.execute('UPDATE owned_vehicles SET garage_id = @garage_id, impound = @impound WHERE plate = @plate', {
                ['@plate'] = plate,
                ['@garage_id'] = "impound",
                ['impound'] = 1
            })
